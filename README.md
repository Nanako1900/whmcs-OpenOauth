# Nanako OAuth2 Login for WHMCS

WHMCS addon module that integrates a self-hosted Nanako OAuth2 server for third-party login. Supports client login/registration, account binding/unbinding, and admin panel OAuth login.
...
- PHP 8.1+
- cURL extension
- A Nanako OAuth2 server with the following endpoints:
...
3. Find **Nanako OAuth2 Login** and click **Activate**.
...
| Login Button Text | Text displayed on the login button | `使用 Nanako 账号登录` |
